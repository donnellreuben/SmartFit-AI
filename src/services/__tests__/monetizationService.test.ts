...
jest.mock('../analyticsService', () => ({
  analyticsService: {
    trackEvent: jest.fn(),
    createABTest: jest.fn(() => Promise.resolve('test-id')),
  },
}));
...
describe('MonetizationService', () => {
  ...
  describe('getInAppPurchaseById', () => {
    it('should return purchase for valid ID', () => {
      const purchases = monetizationService.getInAppPurchases();
      if (purchases.length > 0) {
        const purchase = monetizationService.getInAppPurchaseById(
          purchases[0].id,
        );
  ...
  describe('processInAppPurchase', () => {
    it('should process valid purchase', async () => {
      const purchases = monetizationService.getInAppPurchases();
      if (purchases.length > 0) {
        const result = await monetizationService.processInAppPurchase(
          purchases[0].id,
        );
  ...
  describe('updateStrategyStatus', () => {
    it('should update strategy status', async () => {
      ...
      if (strategies.length > 0) {
        await expect(
          monetizationService.updateStrategyStatus(
            strategies[0].id,
            'completed',
          ),
